...
| B-new-17 | **Role enforcement — backend guard cho claim + post endpoints** | _unassigned_ | [ ] | **Bug:** user có `role_type='giver'` hiện vẫn gọi được `POST /posts/:id/claims`. Fix: (1) claim endpoint check `caller.role_type IN ('receiver','both')` → 403 `forbidden_role` nếu giver; (2) post creation `POST /posts` check `caller.role_type IN ('giver','both')` → 403 `forbidden_role` nếu receiver. **Edge cases:** (a) business staff: business identity luôn là giver, nhưng member vẫn có personal `role_type` riêng → guard phải check `role_type` của user, không phải membership. (b) org claim (B8-10): khi claim dưới danh nghĩa org, org phải là `receiver`-type org (verified). (c) new user chưa có `role_type` (NULL): cần block hết cho đến khi B-new-18 hoàn thành. |
| B-new-18 | **Role onboarding endpoint** — `POST /auth/complete-profile` hoặc extend `PUT /me` để set `role_type` lần đầu | _unassigned_ | [ ] | Khi user đăng ký lần đầu qua social auth (B6-1), `role_type` hiện để NULL. Cần: field `role_type` trong `PUT /me` (giver / receiver / both), validate chỉ nhận 3 giá trị đó; trả 422 `invalid_role` nếu sai. **Lưu ý:** `role_type='both'` không nên dễ chọn trên UI (tránh abuse) — chỉ cho phép user toggle thêm role sau này (xem F-new-11). Phụ thuộc B-new-17 (để null-role bị block đúng). |
...
| F-new-9 | **Role gating UI — ẩn/disable actions theo role_type của user đang đăng nhập** | _unassigned_ | [ ] | Phụ thuộc B-new-17. Đọc `role_type` từ `GET /me` response (lưu trong `AuthService`). Rules: (1) `role_type='giver'` → ẩn "Nhận" button ở Item Detail, ẩn tab "Đồ nhận" trong Profile, ẩn Messages thread từ phía receiver. (2) `role_type='receiver'` → ẩn FAB "Đăng đồ" ở Home Feed, ẩn "Đồ tôi đăng" trong Profile, ẩn tab Doanh nghiệp/Tổ chức trong Profile. (3) `role_type='both'` → show all. (4) `role_type=NULL` (new user chưa chọn) → block cả hai action, redirect tới F-new-10 onboarding. |
| F-new-10 | **Role onboarding screen** — màn hình chọn vai trò sau lần đăng nhập đầu tiên | _unassigned_ | [ ] | Phụ thuộc B-new-18 + F-new-9. Trigger: `AuthService` detect `role_type == null` sau login → push `RoleSelectionScreen` trước khi vào Home Feed. UI: 2 lựa chọn rõ ràng — "Tôi muốn cho đồ" (giver) và "Tôi muốn nhận đồ" (receiver), với option "Cả hai" nhỏ hơn phía dưới. Sau chọn → `PUT /me {role_type: ...}` → lưu vào AuthService → navigate Home. Không cho phép skip. |
| F-new-11 | **Role toggle trong Settings — cho phép thêm role sau** | _unassigned_ | [ ] | Phụ thuộc B-new-18. UX: giver muốn bắt đầu nhận → Settings có nút "Tôi cũng muốn nhận đồ" → confirm dialog giải thích → `PUT /me {role_type: 'both'}`. Tương tự cho receiver muốn thêm giver role. **Lưu ý:** chỉ cho thêm role (giver → both, receiver → both), không cho downgrade (both → giver) vì có thể ảnh hưởng pending claims/posts. |
